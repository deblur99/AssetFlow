...
struct WelcomeView: View {
    ...
    private func openAndCloseWelcome(_ action: @MainActor () -> Void) {
        action()
        // NSApp.keyWindow 대신 AppDelegate.welcomeWindow를 직접 닫는다.
        // async Task(파일 선택 패널 등)에서 호출될 때 NSApp.keyWindow가 nil이거나
        // 이미 열린 프로젝트 창이 될 수 있어 신뢰할 수 없다.
        (NSApp.delegate as? AppDelegate)?.closeWelcomeWindow()
    }
}
